...
def main():
    ...
    for exp_name, config in dict_experiments.items():
        ...
        for asset in ASSETS:
            ...
            path_results = PATH_REPORTS + f'test_results/LSTM_with_Attention_{asset.replace(".", "_")}_features={"_".join(feature_cols)}__label={"_".join(label_col)}__sql_len={seq_len}_test_results.csv'
            ...
            model.fit(X_train, y_train, epochs=1000, batch_size=25, validation_split=0.2, callbacks=callbacks)
